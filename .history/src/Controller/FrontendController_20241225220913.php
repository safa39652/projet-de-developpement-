...
#[Route('/rapport-evenements', name: 'rapport_evenements', methods: ['GET', 'POST'])]
    ...
    public function generateReport(Request $request, PointageRepository $pointageRepository, EventRepository $eventRepository): Response
    {
        ...
        // Filtrer les données en fonction du type de rapport
        if ($reportType === 'presence') {
            // Générer le rapport de présence
            $pointages = $this->getPointagesByFilter($pointageRepository, $dateFilter, $startDate, $endDate);
            $spreadsheet = $this->generatePresenceSpreadsheet($pointages);
            $fileName = 'rapport_presence.xlsx';
        } elseif ($reportType === 'event') {
            // Générer le rapport des événements
            $events = $this->getEventsByFilter($eventRepository, $dateFilter, $startDate, $endDate);
    ...
    private function getPointagesByFilter(PointageRepository $pointageRepository, string $dateFilter, string $startDate, string $endDate)
    {
        // Filtrer les données en fonction du type de filtre choisi
        if ($dateFilter === 'week') {
            return $pointageRepository->findByWeek($startDate, $endDate);
        } elseif ($dateFilter === 'month') {
            return $pointageRepository->findByMonth($startDate, $endDate);
        } elseif ($dateFilter === 'year') {
            return $pointageRepository->findByYear($startDate, $endDate);
        } else {
    ...
    private function getEventsByFilter(EventRepository $eventRepository, string $dateFilter, string $startDate, string $endDate)
    {
        // Filtrer les données des événements en fonction du type de filtre choisi
        if ($dateFilter === 'week') {
            return $eventRepository->findByWeek($startDate, $endDate);
        } elseif ($dateFilter === 'month') {
            return $eventRepository->findByMonth($startDate, $endDate);
        } elseif ($dateFilter === 'year') {
            return $eventRepository->findByYear($startDate, $endDate);
        } else {
            return $eventRepository->findAll();
        }
    }

    private function generateEventSpreadsheet(array $events)
    {
        // Générer le rapport des événements (Excel)
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $headers = ['ID', 'Titre', 'Date Début', 'Date Fin', 'Durée (en heures)'];
        $this->generateSpreadsheetData($sheet, $headers, $events);
        return $spreadsheet;
    }
