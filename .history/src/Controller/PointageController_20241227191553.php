    ...
    public function rapportPointages(Request $request, EntityManagerInterface $entityManager): Response
{
    ...
    // Appliquer le filtre par jour (jour de la semaine)
    if ($jour) {
        $qb->andWhere('p.heureEntree = :jour')
            ->setParameter('jour', $jour); // Cela suppose que vous filtrez par une date exacte ou un jour de la semaine
    }

    // Exécuter la requête
    $pointages = $qb->getQuery()->getResult();

    return $this->render('rapport/pointages.html.twig', [
        'pointages' => $pointages,
    ]);
}
}
