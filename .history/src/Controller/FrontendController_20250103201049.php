...
class FrontendController extends AbstractController
{
    ...
    public function __construct(EntityManagerInterface $entityManager,HttpClientInterface $httpClient, EmployeRepository $employeRepository)
    {
    ...
    #[Route('/api/process', name: 'api_process', methods: ['POST'])]
public function process(Request $request, EntityManagerInterface $entityManager): JsonResponse
{
    ...
    try {
        $response = $this->httpClient->request('POST', 'http://localhost:5000/process', [
            ...
            'body' => ['image' => fopen($uploadsDirectory . '/' . $fileName, 'r')],
        ]);
        ...
        if ($response->getStatusCode() === 200) {
            ...
            $employeeConfidence = $apiResponse['employee']['confidence'];
            $emotionLabel = $apiResponse['emotion']['label'];

            $employe = $entityManager->getRepository(Employe::class)->findOneBy(['nom' => $employeeName]);

            if ($employe) {
                $pointage = new Pointage();
                $pointage->setEmploye($employeeName);
                $pointage->setHeureEntree(new \DateTime());
                $pointage->setStatut($employeeConfidence > 0.5 ? 'Présent' : 'Non reconnu');
                $pointage->setPhoto($imageUrl);
                $entityManager->persist($pointage);

                $emotion = new Emotion();
                $emotion->setEmploye($employe);
                $emotion->setEmotion($emotionLabel);
        ...
        return new JsonResponse(['message' => 'Erreur interne : ' . $e->getMessage()], 500);
    }

    return new JsonResponse(['message' => 'Erreur lors du traitement'], 500);
}
