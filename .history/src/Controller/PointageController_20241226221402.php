...
class PointageController extends AbstractController
{
    ...
    public function enregistrerPointage(Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        $pointage = $entityManager->getRepository(Pointage::class)
                                  ->findOneBy(['employe' => $employe, 'heureSortie' => null]);
        ...
        if ($pointage) {
            ...
            // Sinon, il s'agit d'un pointage d'entrée
            $pointage = new Pointage();
            $pointage->setEmploye($data['employe']);
            $pointage->setHeureEntree(new \DateTime());
            $pointage->setStatut($data['statut'] ?? null);
            $pointage->setPhoto($data['photo'] ?? null);

            // Calculer le jour de la semaine
            $jour = (new \DateTime())->format('l');
            $pointage->setJour($jour);

            // Déterminer le statut (retard, sortie avant l'heure, présence, absent)
