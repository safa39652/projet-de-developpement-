...
public function enregistrerPointage(Request $request, EntityManagerInterface $entityManager): JsonResponse
{
    ...
    public function rapportPointages(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        // Récupérez le jour de la semaine à partir de la date fournie
        $jour = $jourDeLaSemaine[date('l', strtotime($dateDebut))];
        
        // Requête SQL sans DAYOFWEEK
        $sql = 'SELECT * FROM pointage p WHERE EXTRACT(DAY FROM p.heure_entree) = :jour';
