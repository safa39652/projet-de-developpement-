    ...
    public function dashboard(EmployeRepository $employeRepository, PointageRepository $pointageRepository): Response
{
    ...
    public function exportExcel(EmployeRepository $employeRepo, PointageRepository $pointageRepo)
    {
        ...
        foreach ($pointages as $pointage) {
            ...
            // Déboguer l'objet employe pour vérifier sa valeur
            if ($employe) {
                if (is_object($employe)) {
                    // Déboguer pour voir le type de l'objet et la méthode getNom
                    var_dump(get_class($employe)); // Vérifie la classe de l'objet
                    if (method_exists($employe, 'getNom')) {
                        $sheet->setCellValue('A' . $row, $employe->getNom());
                    } else {
                        $sheet->setCellValue('A' . $row, 'Méthode getNom() manquante');
                    }
                } else {
                    $sheet->setCellValue('A' . $row, 'Employé non valide');
                }
            } else {
                $sheet->setCellValue('A' . $row, 'Nom indisponible');
            }
        
            $sheet->setCellValue('B' . $row, $pointage->getHeureEntree());
            $sheet->setCellValue('C' . $row, $pointage->getHeureSortie() ?: 'Non définie');
            $sheet->setCellValue('D' . $row, 'Présent');
            $row++;
        }
