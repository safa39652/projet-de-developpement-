    ...
    private function generateSpreadsheetData($sheet, $headers, $pointages)
{
    ...
    foreach ($pointages as $pointage) {
        $sheet->setCellValue('A' . $row, $pointage->getId());
        $sheet->setCellValue('B' . $row, $pointage->getEmployee()?->getName() ?? 'Non disponible');
        $sheet->setCellValue('C' . $row, $pointage->getCheckInTime()?->format('Y-m-d H:i:s') ?? 'Non disponible');
        $sheet->setCellValue('D' . $row, $pointage->getCheckOutTime()?->format('Y-m-d H:i:s') ?? 'Non disponible');
        $sheet->setCellValue('E' . $row, $pointage->getStatus() ?? 'Non disponible');
        $sheet->setCellValue('F' . $row, $pointage->getMonth() ?? 'Non disponible');
        $sheet->setCellValue('G' . $row, $pointage->getYear() ?? 'Non disponible');
        $sheet->setCellValue('H' . $row, $pointage->getDay() ?? 'Non disponible');
        $row++;
    }
}
