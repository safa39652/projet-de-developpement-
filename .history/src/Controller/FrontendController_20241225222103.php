...
public function generateReport(Request $request, PointageRepository $pointageRepository, EventRepository $eventRepository): Response
{
    ...
    public function generateEventSpreadsheet($events): Spreadsheet
    {
        ...
        foreach ($events as $event) {
            ...
            $sheet->setCellValue('C' . $row, $event->getStartDat() ? $event->getStartDat()->format('Y-m-d H:i:s') : 'Non disponible');
            ...
            if ($event->getDateDebut() && $event->getEndDate()) {
                $duration = $event->getDateDebut()->diff($event->getEndDate())->h;
            } else {
