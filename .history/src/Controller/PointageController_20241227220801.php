...
public function enregistrerPointage(Request $request, EntityManagerInterface $entityManager): JsonResponse
{
    ...
    public function rapportPointages(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        $jour = $request->query->get('jour'); // Jour de la semaine en français (ex: "lundi")

        $connection = $entityManager->getConnection();
        $sql = 'SELECT * FROM pointage p WHERE 1=1'; // Condition de base
        $params = [];
        ...
        if ($jour) {
            ...
            if (isset($jourDeLaSemaine[$jour])) {
                $sql .= ' AND DAYOFWEEK(p.heure_entree) = :jour';
                $params['jour'] = $jourDeLaSemaine[$jour];
            } else {
                // Affichage d'un message de débogage dans les logs ou exception
                throw new \InvalidArgumentException('Le jour spécifié est invalide. Vérifiez que le paramètre "jour" est bien l’un des suivants : dimanche, lundi, mardi, mercredi, jeudi, vendredi, samedi.');
            }
        }
    
        // Exécution de la requête
        $stmt = $connection->prepare($sql);
        $pointages = $stmt->executeQuery($params)->fetchAllAssociative();
