    ...
    public function checkEmployee(Request $request): JsonResponse
    {
        try {
            ...
            if (!$file || !$file->isValid()) {
                return new JsonResponse(['message' => 'Aucune image valide reçue'], 400);
            }
    
            // Vérifier le type MIME pour s'assurer que c'est une image
            $allowedMimeTypes = ['image/jpeg', 'image/png', 'image/jpg'];
            if (!in_array($file->getMimeType(), $allowedMimeTypes)) {
                return new JsonResponse(['message' => 'Format de fichier non supporté'], 400);
            }
    
            // Définir le chemin où l'image sera stockée
            $uploadsDirectory = $this->getParameter('kernel.project_dir') . '/public/uploads';
            if (!is_dir($uploadsDirectory)) {
                mkdir($uploadsDirectory, 0777, true); // Créer le dossier si nécessaire
            }
    
            $fileName = uniqid() . '.' . $file->guessExtension(); // Générer un nom unique
            $file->move($uploadsDirectory, $fileName); // Déplacer le fichier téléchargé
    
            // Construire l'URL complète de l'image
            $imageUrl = $request->getSchemeAndHttpHost() . '/uploads/' . $fileName;
    
            // Appeler l'API Flask pour la reconnaissance faciale
            $faceRecognitionResponse = $this->httpClient->request('POST', 'http://localhost:5000/recognize', [
                'headers' => [
            ...
            $faceRecognitionStatusCode = $faceRecognitionResponse->getStatusCode();
    
            if ($faceRecognitionStatusCode !== 200) {
                $errorMessage = $faceRecognitionResponse->getContent(false);
                return new JsonResponse(['message' => 'Erreur lors de la reconnaissance faciale', 'details' => $errorMessage], $faceRecognitionStatusCode);
            }
    
            $faceData = $faceRecognitionResponse->toArray();
    
            // Appeler l'API Flask pour la détection des émotions
            $emotionDetectionResponse = $this->httpClient->request('POST', 'http://localhost:5000/emotion', [
                'headers' => [
                    'Content-Type' => 'multipart/form-data',
                ],
                'body' => [
            ...
            $emotionDetectionStatusCode = $emotionDetectionResponse->getStatusCode();
    
            if ($emotionDetectionStatusCode !== 200) {
                $errorMessage = $emotionDetectionResponse->getContent(false);
                return new JsonResponse(['message' => 'Erreur lors de la détection des émotions', 'details' => $errorMessage], $emotionDetectionStatusCode);
            }
    
            $emotionData = $emotionDetectionResponse->toArray();
    
            // Si un employé est reconnu, récupérer ses données dans la base
            if (isset($faceData['employee'])) {
                $employeeName = $faceData['employee']['name'];
                ...
                if ($employe) {
                    ...
                    $faceData['employee']['poste'] = $employe->getPoste(); // Poste
                    $faceData['employee']['id'] = $employe->getId();     // ID
                } else {
                    $faceData['employee']['poste'] = 'Inconnu';
                    $faceData['employee']['id'] = 'Inconnu';
                }
            }
    
            // Ajouter les données de détection des émotions à la réponse
            $faceData['emotion'] = $emotionData;
    
            // Ajouter l'URL de l'image capturée au retour JSON
            $faceData['capturedImageUrl'] = $imageUrl;
    
            return new JsonResponse($faceData);
    
        } catch (\Exception $e) {
            // Gestion des erreurs générales
            return new JsonResponse(['message' => 'Erreur interne', 'details' => $e->getMessage()], 500);
        }
    }
