...
public function generateReport(Request $request, PointageRepository $pointageRepository, EventRepository $eventRepository): Response
{
    ...
    public function generateEventSpreadsheet($events): Spreadsheet
    {
        ...
        foreach ($events as $event) {
            ...
            if ($event->getStartDate() && $event->getEndDate()) {
                $duration = $event->getStartDate()->diff($event->getEndDate())->h;
            } else {
    ...
    public function generateStyledEventSpreadsheet($events): Spreadsheet
{
    $spreadsheet = new Spreadsheet();
    $sheet = $spreadsheet->getActiveSheet();
    ...
    // Appliquer un style aux en-têtes : couleur de fond et alignement
    $sheet->getStyle('A1:E1')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('4CAF50'); // Vert
    $sheet->getStyle('A1:E1')->getFont()->getColor()->setRGB('FFFFFF'); // Texte blanc
    $sheet->getStyle('A1:E1')->getFont()->setBold(true); // Mettre en gras
    $sheet->getStyle('A1:E1')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER); // Centrer l'alignement horizontal

    // Remplir les données avec des valeurs par défaut et aligner le texte
    $row = 2;
    foreach ($events as $event) {
        $sheet->setCellValue('A' . $row, $event->getId());
        ...
        // Durée (calculée si les dates sont présentes)
        $duration = ($event->getDateDebut() && $event->getDateFin()) ? $event->getDateDebut()->diff($event->getDateFin())->h : 'Non disponible';
        $sheet->setCellValue('E' . $row, $duration);

        // Appliquer un alignement au texte dans chaque cellule
        $sheet->getStyle('A' . $row . ':E' . $row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER); // Centrer les cellules
        $sheet->getStyle('A' . $row . ':E' . $row)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER); // Centrer verticalement
