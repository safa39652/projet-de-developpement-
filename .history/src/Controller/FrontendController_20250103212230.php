...
class FrontendController extends AbstractController
{
    ...
    public function __construct(HttpClientInterface $httpClient, EmployeRepository $employeRepository, LoggerInterface $logger)
    {
    ...
    public function checkEmployee(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $this->logger->info("Réponse de l'API Flask : $statusCode");
            ...
            if ($statusCode === 200) {
                ...
                $this->logger->error("Erreur de l'API Flask, code : $statusCode");
                return new JsonResponse(['message' => 'Erreur lors de la reconnaissance'], $statusCode);
            }
        } catch (\Exception $e) {
            // Log l'erreur si l'API Flask ne répond pas ou si une exception se produit
            $this->logger->error("Erreur lors de la communication avec l'API Flask : " . $e->getMessage());
            return new JsonResponse(['message' => 'Erreur lors de la communication avec l\'API Flask'], 500);
        }
    }
