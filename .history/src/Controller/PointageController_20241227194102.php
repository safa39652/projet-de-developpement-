    ...
    public function rapportPointages(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($jour) {
            ...
            if (isset($jourDeLaSemaine[$jour])) {
                // Utilisation d'une requête SQL native pour le filtrage par jour de la semaine
                $connection = $entityManager->getConnection();
$sql = 'SELECT * FROM pointage p WHERE DAYOFWEEK(p.heureEntree) = :jour';
$stmt = $connection->prepare($sql);
$stmt->execute(['jour' => $jourDeLaSemaine[$jour]]);
$pointages = $stmt->fetchAllAssociative(); // ou fetchAll() si vous ne voulez pas utiliser Associative
        ...
        // Exécuter la requête DQL si aucun filtre par jour
