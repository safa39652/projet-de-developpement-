    ...
    public function checkEmployee(Request $request): JsonResponse
{
    ...
    $faceResponse = $this->httpClient->request('POST', 'http://localhost:5000/recognize', [
        'headers' => [
    ...
    // Récupérer les réponses des API Flask
    $faceStatusCode = $faceResponse->getStatusCode();
    $emotionStatusCode = $emotionResponse->getStatusCode();

    if ($faceStatusCode === 200 && $emotionStatusCode === 200) {
        $faceApiResponse = $faceResponse->toArray();
        $emotionApiResponse = $emotionResponse->toArray();

        // Traiter les données de l'API de reconnaissance faciale
        if (isset($faceApiResponse['employee'])) {
            $employeeName = $faceApiResponse['employee']['name'];

            // Chercher l'employé par le nom détecté
            $employe = $this->employeRepository->findOneBy(['nom' => $employeeName]);

            if ($employe) {
                $faceApiResponse['employee']['poste'] = $employe->getPoste();
                $faceApiResponse['employee']['id'] = $employe->getId();
            } else {
                $faceApiResponse['employee']['poste'] = 'Inconnu';
                $faceApiResponse['employee']['id'] = 'Inconnu';
            }
        }

        // Ajouter les informations des émotions à la réponse
        $faceApiResponse['emotion'] = $emotionApiResponse['emotion'];

        // Ajouter l'URL de l'image capturée
        $faceApiResponse['capturedImageUrl'] = $imageUrl;

        return new JsonResponse($faceApiResponse);
    } else {
        return new JsonResponse([
            'message' => 'Erreur lors du traitement',
            'faceStatusCode' => $faceStatusCode,
            'emotionStatusCode' => $emotionStatusCode,
        ], max($faceStatusCode, $emotionStatusCode));
    }
}
