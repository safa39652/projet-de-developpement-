...
public function enregistrerPointage(Request $request, EntityManagerInterface $entityManager): JsonResponse
{
    ...
    public function rapportPointages(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        // Filtrer par mois et année dans le code PHP
        if ($mois) {
            $pointages = array_filter($pointages, function ($pointage) use ($mois) {
                return $pointage->getHeureEntree()->format('m') == $mois;
            });
        }
    
        if ($annee) {
            $pointages = array_filter($pointages, function ($pointage) use ($annee) {
                return $pointage->getHeureEntree()->format('Y') == $annee;
            });
        }
